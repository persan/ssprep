...
procedure Checkrelease is
   ...
   begin
      Put_Line
        (Command_Name & " "  & test0002.Version.Full_Version & Lf &
         "Syntax:" & Lf &
   ...
   begin
      ...
      while not End_Of_File (F) loop
         ...
         begin
            if Index (S, Now) /= 0 and Line (F) < 20 then
               Date_Ok := True;
            elsif Index (S, test0002.Version.Version ) /= 0 and Line (F) < 20 then
               Version_Ok := True;
            end if;
         end;
      end loop;
      ...
      if not Version_Ok then
         Put_Line ("Current version '" & test0002.Version.Version  & "' is not mentioned in the relese note");
         Set_Exit_Status (Failure);
      end if;
   ...
   begin
      ...
      while not End_Of_File (F) loop
         declare
            S : constant String := Get_Line (F);
         begin
            if test0002.Version.Version = S (S'First .. S'Last - 1) then
               Put_Line ("Version " &  test0002.Version.Version & " does alredy exists.");
               Set_Exit_Status (Failure);
            end if;
         end;
      end loop;
   ...
   begin
      loop
         case Getopt ("-dirtylist= -svnversions= -releasenote= v ? h -help") is
            ...
            when 'v' =>
               Put_Line (test0002.Version.Version);
            when '?' | 'h' =>
               Help;
               return;
            when others =>
